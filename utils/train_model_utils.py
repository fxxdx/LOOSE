...
class ModelExecutioner:
	...
	def train_one_epoch(self, epoch_index, tb_writer, training_loader):
		...
		for i, (inputs, mask, labels) in loop:
			...
			outputs = self.model(inputs.float()).logits.to(self.device)
			...
			# print("i and accuracy: ", i, accuracy)
	...
	def train(self, n_epochs, training_loader, validation_loader, verbose=True):
		...
		Path(os.path.join(self.weights_dir, self.model_name, f"horizon_{self.horizon}")).mkdir(parents=True, exist_ok=True)
