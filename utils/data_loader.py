...
class DataLoader:
	...
	def get_dataset_names(self):
		...
		return [x for x in names if os.path.isfile(os.path.join(self.data_path, x))]
	...
	def load(self, dataset):
		...
		for name in pbar:
			...
			for fname in glob.glob(os.path.join(self.data_path, name)):
				# print("fname: ", fname)
				curr_data = pd.read_csv(fname).to_numpy()
				# print(curr_data)
				if curr_data.ndim != 2:
