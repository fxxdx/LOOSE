...
class EmbeddingStem(nn.Module):
	def __init__(
		...
		if linear_patch or conv_patch:
			...
			self.pos_embed = nn.Parameter(torch.zeros(1, self.num_windows,embedding_dim))
	...
	def forward(self, x):
		print("x:", x.shape)
		if self.linear_patch or self.conv_patch:
			self.pos_embed = nn.Parameter(
				torch.zeros(1, self.num_windows, x.shape[-1])
			)
		if self.conv_stem:
			x = self.projection(x)
		...
		return self.pos_drop(x + self.pos_embed.to("cuda"))
