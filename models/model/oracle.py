...
class Oracle:
	...
	def fit(self, metric):
		...
		print("forcast_names: ", forcast_names)
		data = data[forcast_names]
		print("data: ", data)
		fnames = data.index
		data = data.to_numpy()
		...
		print("score[coin]:", score[coin])
		if self.randomness == 'true':
			score[inv_coin] = data[inv_coin, random_choices[inv_coin]]
			print("score[inv_coin]:", score[inv_coin])
			print("inv_cpin:", inv_coin)
		else:
			data.sort(axis=1)
