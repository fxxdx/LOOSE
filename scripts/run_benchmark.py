...
if __name__ == "__main__":
    ...
    if args.report_method == "csv":
        ...
        horizondata = json.loads(args.strategy_args)
        # pattern = r'\d+'
        # horizon= re.findall(pattern, horizon)
        horizon = horizondata['horizon']
        # if args.model_hyper_params[0] != None:
        #     seqlength = json.loads(args.model_hyper_params[0])
        #     if !('seq_len' in seqlength):
        #         seqlength = 336
        #     else:
        #         seqlength = seqlength['seq_len']
        # else:
        lookback = 336
        leaderboard_file_name = "test_report_" + str(horizon)  + '_' +str(lookback) + filename
