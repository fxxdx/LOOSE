...
class FixedForecast(ForecastingStrategy):
    ...
    def _execute(
        ...
        seq_len = 336
        sequence = seq_len + horizon
        result_list = []
        # for i in range(int(data_len/sequence)):
        #     data = series.iloc[i*sequence:(i+1)*sequence]
        #     print("data shape :", i, data.shape)
        #     train_length = sequence - horizon
        #     train_valid_data, test_data = split_before(data, train_length)
        #     print("train_valid_data shape: ", train_valid_data.shape)
        ...
        #     )
        #     print("single_series_results: ", single_series_results)
        ...
        if_time = end_inference_time - end_fit_time,
        single_series_results, log_info = self.evaluator.evaluate_with_log(
            test_data.to_numpy(),
            predicted,
            scaler=self._get_eval_scaler(train_valid_data, train_ratio_in_tv),
            hist_data=train_valid_data.values,
        )
        inference_data = pd.DataFrame(
            predicted, columns=test_data.columns, index=test_data.index
        )
